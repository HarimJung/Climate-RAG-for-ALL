      ...
      <div className="mx-auto max-w-[1100px]">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {SECTIONS.map((section) => (
            ...
            >
              <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-blue-50 text-[--accent-primary]">
                <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" d={section.iconPath} />
                </svg>
              </div>
              <h2 className="text-[17px] font-bold text-[--text-primary] transition-colors group-hover:text-[--accent-primary]">
                {section.title}
              </h2>
        ...
        <section className="mt-14 rounded-2xl border border-[--border-card] bg-[--bg-section] p-8 text-center">
          <p className="text-lg font-bold text-[--text-primary]">Put your knowledge into action</p>
          <p className="mt-1.5 text-[14px] text-[--text-secondary]">Search any country to see its climate report card, or download a poster to share.</p>
